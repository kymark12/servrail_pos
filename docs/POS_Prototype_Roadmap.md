...
**Vendor vs. client certification split — see `BIR_Certification_Playbook.md`** for the full two-tier process: ServRail holds the one-time system-level Certificate of Accreditation (vendor tier), each client separately files their own Permit to Use per branch (client tier). That doc also flags the open question of whether BIR will accredit a "bring your own hardware" software model or require named reference tablet/printer configs — unresolved as of 2026-07-15, confirm before committing hardware flexibility to a client.
